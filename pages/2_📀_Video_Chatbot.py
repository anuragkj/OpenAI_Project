...
async def main():
    ...
    async def conversational_chat(query):
        ...
        qa = ConversationalRetrievalChain.from_llm(ChatOpenAI(model_name="gpt-3.5-turbo"), retriever=vectors.as_retriever(),
    ...
    if 'output' in st.session_state:
        ...
        # uploaded_file = st.file_uploader("Choose a file (Generate report from Homepage if help required over the
        # video)", type="pdf")

        # if st.button("Generate Chatbot"):

        # Add your code here that needs to be executed pdf = PyPDF2.PdfFileReader() qa =
        # ConversationalRetrievalChain.from_llm(ChatOpenAI(model_name="gpt-3.5-turbo"),
        ...
        if st.session_state['ready']:
            ...
            with container:
                with st.form(key='my_form', clear_on_submit=True):
                    user_input = st.text_input("Query:", placeholder="e.g: Summarize the video in a few sentences",
                                               key='input')
